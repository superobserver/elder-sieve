  ...
  if y >= new_floor:
    if y <= new_test:
      ...
      try:
        ...
        #composites_base10.append((y*90)+primitive_var)
      except:
        pass
  for n in xrange (1, new_test):
    new_y = y+((z+(90*(x-1)))*n)
    if new_y <= new_test and new_y >= new_limit:
      try:
        #composites_base10.append((new_y*90)+primitive_var)
        list_var[new_y-new_floor] = 0# z #0
      except:
        pass
    new2_y = y+((o+(90*(x-1)))*n)   
    if new2_y <= new_test and new_y >= new_floor:
      try:    
        #composites_base10.append((new2_y*90)+primitive_var)
...
for x in xrange(1, 336): # 10000 = 12; 100000 = 36; 1,000,000 = 108; 10,000,000 = 336; 100,000,000 = 1056; 1,000,000,000 = "Hey I need to checksum these tables, all hand-entered
